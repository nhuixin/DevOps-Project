...
public class ProfileSuccessServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
			throws ServletException, IOException {
		// step1 - initialize a printwriter object to return the html values via the
		// response
		...
		try {
			...
			Connection con = DriverManager.getConnection("jdbc:mysql://localhost:3306/profile_details", "root",
					"password");
			...
			// Step 7: check if the query had been successfully execute, return
			// �Successfully added profile details!
			// � via the response,
			if (i > 0) {
				PrintWriter writer = response.getWriter();
				writer.println("<h1>" + "Successfully added profile details!" + "</h1>"
						+ "<a href = 'http://localhost:8091/DevOpsProject/ProfileServlet/dashboard' class='nextpagebtn'>"
						+ "next" + "</a>");
				writer.close();
			}
		}
